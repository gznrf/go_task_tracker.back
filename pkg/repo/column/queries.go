...
var (
	...
	updateQuery       = fmt.Sprintf(`UPDATE %s SET board_id = $1, name = $2 WHERE id = $3 RETURNING id`, app.ColumnTable)
	deleteQuery       = fmt.Sprintf(`DELETE FROM %s WHERE id = $1 RETURNING id`, app.ColumnTable)
)
